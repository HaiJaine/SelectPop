...
3. 在“划词设置”里选择触发模式、黑白名单、工具条偏移和工具条大小
...
“工具条位置”里除了偏移和自动消失时间外，还支持通过预设档位和百分比微调工具条大小。当前默认值已经比旧版更紧凑，如果你希望更大或更小，可以直接在这里调整。

“高风险禁用项”是安全兜底，不是功能前提。默认建议保持开启，用来避免在游戏、远控、截图工具、全屏独占窗口和安全敏感软件里误触发划词；只有你明确希望在这些场景里也取词时，再关闭对应项。
...
- 黑名单或高风险禁用项是否屏蔽了当前程序
- 原生 helper 是否启动失败
- VS Code / Cursor、终端类控件以及其他复杂软件中的划词结果优先走原生 helper 的可访问性读取；当前版本不会再通过 `Ctrl+C` / `Ctrl+Shift+C` 取词，常规接口失败时会直接保留原生失败诊断
